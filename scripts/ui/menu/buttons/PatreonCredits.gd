...
#func _ready():
	...
	#
	#
	#if file.open("res://patreon3.txt",FileAccess.READ) == OK: # will be FILE_NOT_FOUND if it doesn't exist
		#p3 = file.get_as_text().split("\n",false)
		#file.close()
	#
	#if file.open("res://patreon2.txt",FileAccess.READ) == OK:
		#p2 = file.get_as_text().split("\n",false)
		#file.close()
	#
	#if file.open("res://patreon1.txt",FileAccess.READ) == OK:
