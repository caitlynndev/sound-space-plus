...
func _input(event): # any unicode input starts search
	...
	if get_viewport().gui_get_focus_owner() == self: return
	if get_viewport().gui_get_focus_owner() == $"/root/Menu/Main/Maps/MapRegistry/T/AuthorSearch": return
	if get_viewport().gui_get_focus_owner() == $"/root/Menu/Main/Maps/Results/Results/RS/H2/Mods/SpeedMod/C/CustomSpeed".get_line_edit(): return
	if get_viewport().gui_get_focus_owner() == $"/root/Menu/Main/Maps/Results/Results/RS/H2/Mods/StartOffset/TimeTextBox": return
