...
#		print("start layer %s" % layer)
		...
		while subfolders.size() != 0:
			...
			if err == OK:
				...
				if err == OK:
					...
					var p:String = cpath.path_join(n)
					while n:
						i += 1
						if pause_amt != -1 and (pause_amt == 0 or ((i%pause_amt) == 0)):
							await get_tree().idle_frame
	...
	print("-- end recurse - took %s usec --" % [Globals.comma_sep(Time.get_ticks_usec() - a)])
	if pause_amt != -1:
...
func _process(delta):
	...
	if Input.is_action_just_pressed("console"):
		if !console_open:
			...
			con.set("theme_override_fonts/font",load("res://assets/font/console.tres"))
...
func _ready():
	...
	confirm_prompt = load("res://prefabs/menu/confirm.tscn").instantiate()
	rootg.call_deferred("add_child",confirm_prompt)

	string_prompt = load("res://prefabs/menu/string.tscn").instantiate()
	rootg.call_deferred("add_child",string_prompt)
	
	file_sel = load("res://prefabs/menu/filesel.tscn").instantiate()
	rootg.call_deferred("add_child",file_sel)
	
	notify_gui = load("res://prefabs/menu/notification_gui.tscn").instantiate()
	rootg.call_deferred("add_child",notify_gui)
	
	fps_disp.offset_left = 15
	fps_disp.offset_top = 15
	fps_disp.offset_right = 0
	fps_disp.offset_bottom = 0
	fps_disp.set("theme_override_fonts/font",load("res://assets/font/debug2.tres"))
